...
ggsave("Figures/Compression.pdf", width = 6.5, height=3.7)
```
...
min_data = group_by(data, Description) %>%
...
ggplot(data, aes(x=DataPoints, y=RatioValue, col=Description)) +
  geom_line() +
  geom_point() +
  theme_bw(base_size = 14) +
  xlab("Number of data points (log-10)") +
  ylab("Ratio vs. minimum (log-10)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  guides(col=FALSE) +
...
ggsave("Figures/Genotypes.pdf", width = 6.5, height=3)
```