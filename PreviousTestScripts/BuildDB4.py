...
try:
    ...
    #########################################################
    smartPrint("Compressing sample info")
    #########################################################
    ...
    for i in range(len(samples)):
        ...
        sample = samples[i]
        sampleTableCompressed[sample] = sampleTable[sample]
    sampleTable.commit()
    sampleTableCompressed.commit()

    ########################################################
    smartPrint("Compressing feature info")
    ########################################################
    ...
    for i in range(len(features)):
        ...
        feature = features[i]
        featureTableCompressed[feature] = featureTable[feature]
    featureTable.commit()
    featureTableCompressed.commit()
    ...
    with open("/tmp/1.tsv", 'w') as transposedFile:
        ...
        for feature in features:
            transposedFile.write(feature + featureTableCompressed[feature] + "\n")
    #########################################################################
finally:
    sampleTable.close()
    featureTable.close()
